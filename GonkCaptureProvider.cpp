...
// ICS header
#define image_rect_type image_rect_type4
#define image_rect_struct image_rect_struct4
#define CameraHardwareInterface CameraHardwareInterface_ICS
#include "gonk/CameraHardwareInterfaceICS.h"
#undef CameraHardwareInterface
...
// ICS specific subclass, because we can't cast CameraHardwareInterface_ICS to sp<T> in general
class CameraICS : public CameraHardwareInterface {
  public:
    CameraICS(PRUint32 aCamera = 0) : mOk(false), mCamera(nsnull), mModule(nsnull) {
      if (hw_get_module(CAMERA_HARDWARE_MODULE_ID,
                (const hw_module_t **)&mModule) < 0) {
        return;
      }
      char cameraDeviceName[4];
      snprintf(cameraDeviceName, sizeof(cameraDeviceName), "%d", aCamera);
      mCamera = new CameraHardwareInterface_ICS(cameraDeviceName);
      if (mCamera->initialize(&mModule->common) != OK) {
        mCamera.clear();
        return;
      }
    ...
    status_t startPreview() {
      mCamera->setPreviewWindow(mWindow);
  ...
  protected:
    bool mOk;
    sp<CameraHardwareInterface_ICS> mCamera;
    ...
    CameraImpl(PRUint32 aCamera = 0) : mOk(false), mCamera(nsnull) {
      ...
      switch(getType()) {
        ...
        case CAMERA_DEFAULT:
          ...
          break;
        // we should never end up there. Just adding to prevent compilation warning.
        case CAMERA_ICS:
          printf_stderr("Ouch. ICS not supported by CameraImpl!");
          break;
      }
...
CameraHardwareInterface* CameraHardwareInterface::openCamera(PRUint32 aCamera)  {
  ...
  switch(getType()) {
    ...
    case CAMERA_DEFAULT:
      ...
      break;
    case CAMERA_ICS:
      instance = new CameraICS(aCamera);
      break;
  }
...
GonkCameraInputStream::GonkCameraInputStream() :
  ...
  mHeaderSent(false), mClosed(true), mClosing(false), mIs420p(false), mFrameSize(0), mMonitor("GonkCamera.Monitor")
{
...
GonkCameraInputStream::getNumberOfCameras() {
  ...
  if (!cameraLib) {
    ...
    if (hw_get_module(CAMERA_HARDWARE_MODULE_ID,
      ...
      printf_stderr("getNumberOfCameras : Could not load camera HAL module");
      return 0;
    }
...
GonkCameraInputStream::Init(nsACString& aContentType, nsCaptureParams* aParams)
{
  ...
  mHardware->enableMsgType(android::CAMERA_MSG_PREVIEW_FRAME);
...
void GonkCameraInputStream::doClose() {
  ...
  if (mClosed)
    return;
  mHardware->disableMsgType(android::CAMERA_MSG_ALL_MSGS);
